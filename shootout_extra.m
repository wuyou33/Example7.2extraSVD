...
%
 [~,DP0,QP0] = svd(P); 
 [x1,DP1,QP1] = svd2017(z,R,H,x0,DP0,QP0);
 PSVD   = QP1*DP1*QP1';
...
%
  ...
  rssSVD  = 0; % for SVD (2017) solution
  rssB  = 0; % for Bierman solution
    for i=1:3,
      ...
      rssSVD  = rssSVD + (PSVD(i,j)  - PCF(i,j))^2;
...
%
  ...
  rdSVD(k)  = sqrt(rssSVD/rssCF);
  rdB(k)  = sqrt(rssB/rssCF);
  end;
%
% Plot relative differences versus delta
%
top    = 10^ceil(log10(max([max(rdS),max(rdK),max(rdJ),max(rdP),max(rdC),max(rdB),max(rdSVD)])));
bottom = 10^floor(log10(min([min(rdS),min(rdK),min(rdJ),min(rdP),min(rdC),min(rdB),min(rdSVD)])));
left   = 10^floor(log10(min(d)));
right  = 10^ceil(log10(max(d)));
loglog([right/50000,right/2000],[top/(72900000*30),top/(72900000*30)],'k-*',[right/50000,right/2000],[top/72900000,top/72900000],'r-',[right/50000,right/2000],[top/2430000,top/2430000],'g-',[right/50000,right/2000],[top/81000,top/81000],'b-',[right/50000,right/2000],[top/2700,top/2700],'y-',[right/50000,right/2000],[top/90,top/90],'k-',[right/50000,right/2000],[top/3,top/3],'k:',d,rdJ,'y-',d,rdP,'b-',d,rdC,'g-',d,rdB,'r-',d,rdSVD,'k-*',d,rdK,'k-',d,rdS,'k:');
...
text(right/1000,top/(72900000*30),'SVD(2017)');
text(eps,bottom,'!');
